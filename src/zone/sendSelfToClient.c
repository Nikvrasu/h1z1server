// ============================================================================
// Zone Server — Character Data Helpers
//
// Head type / actor model lookups are now in loginUdp_11.c (LookupHeadType).
// The zone server uses session->pGetPlayerActor which is populated by the
// login server during CharacterCreateRequest. If data is missing, OnLogin
// applies defaults before any packet uses it.
// ============================================================================

// Resource Type Lookup — maps resource IDs to their type constants
...
// ============================================================================
// Gender-aware model name helpers
// ============================================================================
String8 GetChestModel(u32 gender) {
    return (gender == 2) ? STR8("SurvivorFemale_Chest_Bra.adr")
                         : STR8("SurvivorMale_Chest_Bra.adr");
}

String8 GetLegsModel(u32 gender) {
    return (gender == 2) ? STR8("SurvivorFemale_Legs_Pants_Underwear.adr")
                         : STR8("SurvivorMale_Legs_Pants_Underwear.adr");
}

String8 GetEyesModel(u32 gender) {
    return (gender == 2) ? STR8("SurvivorFemale_Eyes_01.adr")
                         : STR8("SurvivorMale_Eyes_01.adr");
}

// ============================================================================
// SendSelfToClient — Structured packet approach
//
// Builds the complete SendSelfToClient packet with proper character data:
// identity, appearance, inventory, loadout, equipment, profiles, resources.
//
// Ref: H1emu zoneserver.ts sendCharacterData() → SendSelfToClient
//
// The client uses this packet to initialize the player character in-world.
// Critical fields: guid, character_id, position, rotation, items1 (inventory),
// equipment_slots, loadout_slots_array, profiles, character_resources.
// ============================================================================

void SendSelfToClient(AppState* app, SessionState* session, int withStats) {
    printf(MESSAGE_CONCAT_INFO("Building SendSelfToClient for 0x%llx\n"),
           (unsigned long long)session->characterId);

    // Resolve appearance data with fallbacks
    ...
    String8 eyesModel = GetEyesModel(gender);
    String8 chestModel = GetChestModel(gender);
    String8 legsModel = GetLegsModel(gender);

    String8 charName = session->characterName;
    if (charName.size == 0) charName = STR8("Unknown");

    // Build identity string (Steam-like ID)
    char identityBuf[32] = {0};
    u64 steamLikeId = 76561197960265728ull + (session->characterId & 0xffffffffull);
    i32 identityLen = snprintf(identityBuf, sizeof(identityBuf), "%llu",
                               (unsigned long long)steamLikeId);
    String8 identityString = session->ticketIdentity;
    if (identityString.size == 0 && identityLen > 0) {
        identityString.data = (u8*)identityBuf;
        identityString.size = (u64)identityLen;
    }

    printf(MESSAGE_CONCAT_INFO("  model=%u gender=%u head=%u name='%.*s'\n"),
           actorModelId, gender, headType,
           (int)charName.size, charName.data);

    // ============================================================
    // Build the SendSelfToClient packet
    // Ref: H1emu packets/ClientProtocol/ClientProtocol_1080/sendSelf.ts
    // ============================================================
    ...
    sendSelf.payload_self = (struct payload_self_s[1]){{
        ...
        .unk_u32_4 = 0,
        .unk_u32_5 = 0,
        // Position — matches ClientBeginZoning spawn point
        ...
        // ============================================================
        // Profile — single profile, type=3 (player)
        // Ref: H1emu uses profile_id=4, type=3 for KotK
        // ============================================================
        .profiles_count = 1,
        ...
        // ============================================================
        // Inventory Items — items1 array
        // Must contain entries for every loadout/equipment slot GUID.
        // Without these, hotbar stays blank.
        // Ref: H1emu character.ts pGetFullCharacterDataSchema()
        // ============================================================
        .items1_count = 5,
        .items1 = (struct items1_s[5]){
            [0] = { // Fists (right hand / melee)
            ...
            [1] = { // Binoculars
                ...
                .container_def_id   = WEAPON_BINOCULARS,
        ...
        .character_stats1 = withStats ? (struct character_stats1_s[1]){
            { .stat_id11 = 2, .stat_id22 = 2, .variable_u8_1_case = 0,
              .variable_u8_1 = { .vartype_1 = { .base = 1, .modifier = 0 } } },
        } : NULL,
        ...
        // ============================================================
        // Equipment Slots — visual equipment on character model
        // Ref: H1emu character.ts pGetEquipment()
        // ============================================================
        ...
        // ============================================================
        // Loadout Slots — KotK character loadout (profile 17)
        // Default: fists, binoculars, hoodie, jeans, sneakers
        // Ref: H1emu character.ts pGetLoadoutSlots()
        // ============================================================
        ...
        // ============================================================
        // Character Resources — health, hunger, hydration, stamina, etc.
        // Ref: H1emu character.ts pGetResources()
        // ============================================================
        .character_resources_count = 9,
        .character_resources = (struct character_resources_s[9]){
            [0] = { .resource_type1 = HEALTHTYPE,    .resource_id = HEALTHID,    .resource_type2 = HEALTHTYPE,    .unk_array_22866_count = 0, .value = 10000 },
            [1] = { .resource_type1 = HUNGERTYPE,    .resource_id = HUNGERID,    .resource_type2 = HUNGERTYPE,    .unk_array_22866_count = 0, .value = 10000 },
            [2] = { .resource_type1 = HYDRATIONTYPE, .resource_id = HYDRATIONID, .resource_type2 = HYDRATIONTYPE, .unk_array_22866_count = 0, .value = 10000 },
            [3] = { .resource_type1 = STAMINATYPE,   .resource_id = STAMINAID,   .resource_type2 = STAMINATYPE,   .unk_array_22866_count = 0, .value = 10000 },
            [4] = { .resource_type1 = VIRUSTYPE,     .resource_id = VIRUSID,     .resource_type2 = VIRUSTYPE,     .unk_array_22866_count = 0, .value = 0     },
            [5] = { .resource_type1 = BLEEDINGTYPE,  .resource_id = BLEEDINGID,  .resource_type2 = BLEEDINGTYPE,  .unk_array_22866_count = 0, .value = 0     },
            [6] = { .resource_type1 = COMFORTTYPE,   .resource_id = COMFORTID,   .resource_type2 = COMFORTTYPE,   .unk_array_22866_count = 0, .value = 5000  },
            [7] = { .resource_type1 = FUELTYPE,      .resource_id = FUELID,      .resource_type2 = FUELTYPE,      .unk_array_22866_count = 0, .value = 0     },
            [8] = { .resource_type1 = CONDITIONTYPE, .resource_id = CONDITIONID, .resource_type2 = CONDITIONTYPE, .unk_array_22866_count = 0, .value = 10000 },
        },
    ...
    // Send via debug packer that validates stream length
