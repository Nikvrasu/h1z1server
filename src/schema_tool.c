...
u32 win32_buffer_load_from_file(char* file_path, u8* buffer, u32 buffer_length) {
    HANDLE file_handle = CreateFile(file_path, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,
...
u32 win32_buffer_write_to_file(char* file_path, u8* buffer, u32 buffer_length) {
    HANDLE file_handle =
        CreateFile(file_path, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
    if (file_handle == INVALID_HANDLE_VALUE) {
    ...
    u32 bytes_written;
    if (!WriteFile(file_handle, buffer, buffer_length, (LPDWORD)&bytes_written, 0)) {
...
#define TOKEN_KINDS                                                                                    \
    TOKEN_KIND(T_Invalid, "Invalid"),                                                                  \
                                                                                                       \
        TOKEN_KIND(T__Keyword_Begin, ""), TOKEN_KIND(T_Group, "group"),                                \
        TOKEN_KIND(T_Packet, "packet"), TOKEN_KIND(T_Endian, "endian"),                                \
        TOKEN_KIND(T_Opcode, "opcode"), TOKEN_KIND(T__Keyword_End, ""),                                \
                                                                                                       \
        TOKEN_KIND(T__Types_Begin, ""),                                                                \
                                                                                                       \
        TOKEN_KIND(T__Special_Type_Begin, ""), TOKEN_KIND(T_List, "list"),                             \
        TOKEN_KIND(T_String, "string"), TOKEN_KIND(T_Bytes, "bytes"), TOKEN_KIND(T_Stream, "stream"),  \
        TOKEN_KIND(T_Uint2b, "uint2b"), TOKEN_KIND(T_Vec4, "vec4"), TOKEN_KIND(T_Vec3, "vec3"),        \
        TOKEN_KIND(T_Switch, "switch"), TOKEN_KIND(T_Case, "case"),                                    \
        TOKEN_KIND(T__Special_Type_End, ""),                                                           \
                                                                                                       \
        TOKEN_KIND(T__Integral_Begin, ""), TOKEN_KIND(T_B8, "b8"), TOKEN_KIND(T_B16, "b16"),           \
        TOKEN_KIND(T_B32, "b32"), TOKEN_KIND(T_B64, "b64"), TOKEN_KIND(T_I8, "i8"),                    \
        TOKEN_KIND(T_I16, "i16"), TOKEN_KIND(T_I32, "i32"), TOKEN_KIND(T_I64, "i64"),                  \
        TOKEN_KIND(T_U8, "u8"), TOKEN_KIND(T_U16, "u16"), TOKEN_KIND(T_U32, "u32"),                    \
        TOKEN_KIND(T_U64, "u64"), TOKEN_KIND(T_F32, "f32"), TOKEN_KIND(T_F64, "f64"),                  \
        TOKEN_KIND(T__Integral_End, ""),                                                               \
                                                                                                       \
        TOKEN_KIND(T__Endian_Type_Begin, ""), TOKEN_KIND(T_Endian_Little, "little"),                   \
        TOKEN_KIND(T_Endian_Big, "big"), TOKEN_KIND(T__Endian_Type_End, ""),                           \
                                                                                                       \
        TOKEN_KIND(T__Types_End, ""),                                                                  \
                                                                                                       \
        TOKEN_KIND(T_Open_Brace, "Open_Brace"), TOKEN_KIND(T_Close_Brace, "Close_Brace"),              \
        TOKEN_KIND(T_Open_Bracket, "Open_Bracket"), TOKEN_KIND(T_Close_Bracket, "Close_Bracket"),      \
        TOKEN_KIND(T_Colon, "Colon"),                                                                  \
                                                                                                       \
        TOKEN_KIND(T_Identifier, "Identifier"), TOKEN_KIND(T_Number, "Number"),                        \
        TOKEN_KIND(T_Hex_Number, "Hex_Number"),                                                        \
                                                                                                       \
        TOKEN_KIND(T_New_Line, "New_Line"), TOKEN_KIND(T_EOF, "EOF"),                                  \
                                                                                                       \
        TOKEN_KIND(T__End, "")
...
void keyword_hash_map_key_insert(Keyword_Hash_Map* map, char* key, u32 key_length, Token_Kind value) {
    ...
    printf("[*] Inserted %d into bucket %d. Load=%f\n", value, index,
           (f64)map->entry_count / (f64)map->capacity);
#endif
}

Token_Kind keyword_hash_map_value_get(Keyword_Hash_Map* map, char* key, u32 key_length) {
    u64 hash = *(u64*)key;
...
u32 scan_tokens(u8* data, u32 data_length, Token* token_buffer, Keyword_Hash_Map* keyword_map) {
    ...
    for (; byte_count < data_length;) {
        switch (data[byte_count]) {
            case '{': {
                token_buffer[token_buffer_count++] =
                    (Token){ T_Open_Brace, 1, &data[byte_count], line_count };
                byte_count++;
            } break;

            case '}': {
                token_buffer[token_buffer_count++] =
                    (Token){ T_Close_Brace, 1, &data[byte_count], line_count };
                byte_count++;
            } break;

            case '[': {
                token_buffer[token_buffer_count++] =
                    (Token){ T_Open_Bracket, 1, &data[byte_count], line_count };
                byte_count++;
            } break;

            case ']': {
                token_buffer[token_buffer_count++] =
                    (Token){ T_Close_Bracket, 1, &data[byte_count], line_count };
                byte_count++;
            } break;

            case ':': {
                token_buffer[token_buffer_count++] =
                    (Token){ T_Colon, 1, &data[byte_count], line_count };
                byte_count++;
            } break;
            ...
            case '\n': {
                ...
                if (token_buffer_count > 0 && token_buffer[token_buffer_count - 1].kind != T_New_Line) {
                    token_buffer[token_buffer_count++] =
                        (Token){ T_New_Line, 1, &data[byte_count], line_count };
                }
                line_count++;
                byte_count++;
            } break;
                ...
                if (data[byte_count] == '0') {
                    if (data[byte_count + 1] == 'x') {
                        ...
                        for (; is_hex_digit(data[byte_count]); byte_count++)
                            ;
                        token_buffer[token_buffer_count++] =
                            (Token){ T_Hex_Number, byte_count - lexeme_begin, &data[lexeme_begin],
                                     line_count };
                    } else {
                        ...
                        for (; is_digit(data[byte_count]); byte_count++)
                            ;
                        token_buffer[token_buffer_count++] =
                            (Token){ T_Number, byte_count - lexeme_begin, &data[lexeme_begin],
                                     line_count };
                    }
                    ...
                    for (; is_digit(data[byte_count]); byte_count++)
                        ;
                    token_buffer[token_buffer_count++] =
                        (Token){ T_Number, byte_count - lexeme_begin, &data[lexeme_begin], line_count };
                    ...
                    if (byte_count - lexeme_begin <= MAX_KEYWORD_HASH_MAP_KEY_LENGTH) {
                        Token_Kind keyword = keyword_hash_map_value_get(
                            keyword_map, &data[lexeme_begin], byte_count - lexeme_begin);
                        if (keyword) {
#ifdef SCHM_DEBUG
                            printf("[*] ");
...
#endif
                            token_buffer[token_buffer_count++] =
                                (Token){ keyword, byte_count - lexeme_begin, &data[lexeme_begin],
                                         line_count };
                        } else {
                            token_buffer[token_buffer_count++] =
                                (Token){ T_Identifier, byte_count - lexeme_begin, &data[lexeme_begin],
                                         line_count };
                        }
                    } else {
                        token_buffer[token_buffer_count++] =
...
#define PS_KINDS                                                                                       \
    PS_KIND(State_Invalid), PS_KIND(State_Root), PS_KIND(State_Group), PS_KIND(State_Packet),          \
        PS_KIND(State_Basic), PS_KIND(State_List), PS_KIND(State_String), PS_KIND(State_Uint2b),       \
        PS_KIND(State_Stream), PS_KIND(State_Switch), PS_KIND(State_Case), PS_KIND(State_Bytes)
...
struct Parser {
    void (*emit_func)(Parser_Object_Kind kind, Parser_State state_stack[MAX_STATE_STACK],
                      u32 state_stack_tail, Output_Buffers* output_buffers, b32 will_generate_struct,
                      u32 depth);
...
void state_case_enter(Parser* parser) {
    ...
    if (!(parser->tokens[parser->tokens_tail].kind == T_Number
          || parser->tokens[parser->tokens_tail].kind == T_Hex_Number)) {
    ...
    parser->state_stack[parser->state_stack_tail].parent =
        parser->state_stack[parser->state_stack_tail].identifier;
    parser->tokens_tail++;
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Case_Enter, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }

    for (; parser->tokens[parser->tokens_tail].kind != T_Close_Brace
           && parser->tokens[parser->tokens_tail].kind != T_Case;) {
    ...
    // parser->tokens[parser->tokens_tail].line); abort();
    // }
    // parser->tokens_tail++;

    if (parser->emit_func) {
        parser->emit_func(PO_Case_Exit, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
}
...
void state_switch_enter(Parser* parser) {
    ...
    if (parser->tokens[parser->tokens_tail].kind == T_Colon) {
        ...
        if (!(parser->tokens[parser->tokens_tail].kind > T__Integral_Begin
              && parser->tokens[parser->tokens_tail].kind < T__Integral_End)) {
            printf("[X] Expected an integral type after colon at line: %u\n",
                   parser->tokens[parser->tokens_tail].line);
            abort();
        }
        parser->state_stack[parser->state_stack_tail].length_type =
            parser->tokens[parser->tokens_tail].kind;
        parser->tokens_tail++;
    }
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Switch_Enter, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
    ...
    for (; parser->tokens[parser->tokens_tail].kind != T_Close_Brace;) {
        ...
        if (token_kind == T_Case) {
            ...
            parser->depth++;
            parser->state_stack[parser->state_stack_tail].endian =
                parser->state_stack[parser->state_stack_tail - 1].endian;
            parser->state_stack[parser->state_stack_tail].group =
                parser->state_stack[parser->state_stack_tail - 1].group;
            parser->state_stack[parser->state_stack_tail].packet =
                parser->state_stack[parser->state_stack_tail - 1].packet;
            parser->state_stack[parser->state_stack_tail].opcode =
                parser->state_stack[parser->state_stack_tail - 1].opcode;
            parser->state_stack[parser->state_stack_tail].opcode_length_type =
                parser->state_stack[parser->state_stack_tail - 1].opcode_length_type;
            parser->state_stack[parser->state_stack_tail].subcode =
                parser->state_stack[parser->state_stack_tail - 1].subcode;
            parser->state_stack[parser->state_stack_tail].subcode_length_type =
                parser->state_stack[parser->state_stack_tail - 1].subcode_length_type;
            parser->state_stack[parser->state_stack_tail].data_type =
                parser->tokens[parser->tokens_tail];

            state_case_enter(parser);
            memset(&parser->state_stack[parser->state_stack_tail], 0,
                   sizeof(parser->state_stack[parser->state_stack_tail]));
            parser->state_stack_tail--;
            parser->depth--;
        }
    }
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Switch_Exit, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
}
...
void state_stream_enter(Parser* parser) {
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Stream_Enter, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Stream_Exit, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
}
...
void state_list_enter(Parser* parser) {
    ...
    if (parser->tokens[parser->tokens_tail].kind == T_Colon) {
        ...
        if (parser->tokens[parser->tokens_tail].kind > T__Integral_Begin
            ...
            parser->state_stack[parser->state_stack_tail].length_type =
                parser->tokens[parser->tokens_tail].kind;
            parser->tokens_tail++;
        } else if (parser->tokens[parser->tokens_tail].kind == T_Number
                   || parser->tokens[parser->tokens_tail].kind == T_Hex_Number) {
            parser->state_stack[parser->state_stack_tail].count_specifier =
                parser->tokens[parser->tokens_tail];
            parser->tokens_tail++;
        } else {
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_List_Enter, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_List_Exit, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
}
...
void state_string_enter(Parser* parser) {
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Field_String, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
}
...
void state_uint2b_enter(Parser* parser) {
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Field_Uint2b, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
}
...
void state_bytes_enter(Parser* parser) {
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Field_Bytes, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
}
...
void state_field_enter(Parser* parser) {
    switch (parser->tokens[parser->tokens_tail].kind) {
        ...
        case T_Switch: {
            ...
            // state_case_enter(parser);
            // memset(&parser->state_stack[parser->state_stack_tail], 0,
            // sizeof(parser->state_stack[parser->state_stack_tail])); parser->state_stack_tail--;
            // parser->depth--;
            //} break;
        ...
        case T_String: {
            ...
            parser->state_stack[parser->state_stack_tail].data_type =
                parser->tokens[parser->tokens_tail];
            parser->state_stack[parser->state_stack_tail].parent =
                parser->state_stack[parser->state_stack_tail - 1].parent;
        ...
        case T_Vec4: {
            ...
            if (parser->emit_func) {
                parser->emit_func(PO_Field_Vec4, parser->state_stack, parser->state_stack_tail,
                                  parser->output_buffers, 0, parser->depth);
            }
        ...
        case T_Vec3: {
            ...
            if (parser->emit_func) {
                parser->emit_func(PO_Field_Vec3, parser->state_stack, parser->state_stack_tail,
                                  parser->output_buffers, 0, parser->depth);
            }
        ...
        default: {
            ...
            if (parser->emit_func) {
                parser->emit_func(PO_Field_Basic, parser->state_stack, parser->state_stack_tail,
                                  parser->output_buffers, 0, parser->depth);
            }
...
void state_packet_enter(Parser* parser) {
    ...
    if (parser->tokens[parser->tokens_tail].kind > T__Types_Begin
        && parser->tokens[parser->tokens_tail].kind < T__Types_End) {
        will_generate_struct = 1;
    }
    if (parser->emit_func) {
        parser->emit_func(PO_Packet_Enter, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, will_generate_struct, parser->depth);
    }

    // parser->state_stack[parser->state_stack_tail].depth = parser->state_stack_tail;

    for (; !(parser->tokens[parser->tokens_tail].kind == T_Packet
             || parser->tokens[parser->tokens_tail].kind == T_EOF);) {
        if (parser->tokens[parser->tokens_tail].kind > T__Types_Begin
            && parser->tokens[parser->tokens_tail].kind < T__Types_End) {
            state_field_enter(parser);
        } else {
            printf("[X] Expected a type but got a %s at line: %u\n",
                   token_names[parser->tokens[parser->tokens_tail].kind],
                   parser->tokens[parser->tokens_tail].line);
            abort();
        }
    }
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Packet_Exit, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, will_generate_struct, parser->depth);
    }
    memset(&parser->state_stack[parser->state_stack_tail], 0,
           sizeof(parser->state_stack[parser->state_stack_tail]));
...
void state_group_enter(Parser* parser) {
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Group_Enter, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }

    for (; !(parser->tokens[parser->tokens_tail].kind == T_Group
             || parser->tokens[parser->tokens_tail].kind == T_EOF);) {
    ...
    if (parser->emit_func) {
        parser->emit_func(PO_Group_Exit, parser->state_stack, parser->state_stack_tail,
                          parser->output_buffers, 0, parser->depth);
    }
...
u32 write_field_parents(u8* buffer, u32 buffer_tail, Parser_State state_stack[MAX_STATE_STACK],
                        u32 state_stack_tail, u32 depth) {
    u8 parent_buffer[MAX_IDENTIFIER_LENGTH] = { 0 };

    u32 result = 0;
    result += sprintf(buffer + buffer_tail + result, "packet->");
    if (state_stack[state_stack_tail].parent.lexeme_length) {
        for (u32 i = depth; i > 0; i--) {
            memcpy(parent_buffer, state_stack[state_stack_tail - i].parent.lexeme,
                   state_stack[state_stack_tail - i].parent.lexeme_length);
            parent_buffer[state_stack[state_stack_tail - i].parent.lexeme_length] = 0;

            if (state_stack[state_stack_tail - i].kind == State_Switch
                || state_stack[state_stack_tail - i].kind == State_Case) {
                result += sprintf(buffer + buffer_tail + result, "%s.", parent_buffer);
            } else {
                result += sprintf(buffer + buffer_tail + result, "%s[%s_iter].", parent_buffer,
                                  parent_buffer);
            }
        }
    }
...
void emit_c_source(Parser_Object_Kind kind, Parser_State state_stack[MAX_STATE_STACK],
                   u32 state_stack_tail, Output_Buffers* output_buffers, b32 will_generate_struct,
                   u32 depth) {
    u8 identifier_buffer[MAX_IDENTIFIER_LENGTH] = { 0 };
    memcpy(identifier_buffer, state_stack[state_stack_tail].identifier.lexeme,
           state_stack[state_stack_tail].identifier.lexeme_length);
    ...
    memcpy(group_buffer, state_stack[state_stack_tail].group.lexeme,
           state_stack[state_stack_tail].group.lexeme_length);
    ...
    memcpy(packet_buffer, state_stack[state_stack_tail].packet.lexeme,
           state_stack[state_stack_tail].packet.lexeme_length);
    ...
    u8 parent_buffer[MAX_IDENTIFIER_LENGTH] = { 0 };
    memcpy(parent_buffer, state_stack[state_stack_tail].parent.lexeme,
           state_stack[state_stack_tail].parent.lexeme_length);

    switch (kind) {
        case PO_Group_Enter: {
            output_buffers->kind_buffer_tail +=
                sprintf(output_buffers->kind_buffer + output_buffers->kind_buffer_tail,
                        "\n#define %s_PACKET_KINDS \\\n%s_PACKET_KIND(%s_Packet_Kind_Unhandled, "
                        "\"Unhandled\"), \\\n",
                        group_buffer_upper, group_buffer_upper, group_buffer);

            output_buffers->registered_ids_buffer_tail += sprintf(
                output_buffers->registered_ids_buffer + output_buffers->registered_ids_buffer_tail,
                "\nu32 %s_registered_ids[] =\n{\n", group_buffer_lower);

            output_buffers->packer_buffer_tail += sprintf(
                output_buffers->packer_buffer + output_buffers->packer_buffer_tail,
                "\nu32\n%s_packet_pack(%s_Packet_Kind packet_kind, void* packet_ptr, u8* buffer)\n{\n",
                group_buffer_lower, group_buffer);
            ...
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail,
                        "\nvoid\n%s_packet_unpack(u8* data, u32 data_length, %s_Packet_Kind "
                        "packet_kind, void* packet_ptr, Arena* arena)\n{\n",
                        group_buffer_lower, group_buffer);
        ...
        case PO_Group_Exit: {
            output_buffers->kind_buffer_tail += sprintf(
                output_buffers->kind_buffer + output_buffers->kind_buffer_tail,
                "%s_PACKET_KIND(%s_Packet_Kind__End, \"\")\n\ntypedef enum %s_Packet_Kind\n{\n#define "
                "%s_PACKET_KIND(e, s) e\n%s_PACKET_KINDS\n#undef %s_PACKET_KIND\n} %s_Packet_Kind;\n\n",
                group_buffer_upper, group_buffer, group_buffer, group_buffer_upper, group_buffer_upper,
                group_buffer_upper, group_buffer);

            output_buffers->kind_buffer_tail +=
                sprintf(output_buffers->kind_buffer + output_buffers->kind_buffer_tail,
                        "char* %s_packet_names[%s_Packet_Kind__End + 1] =\n{\n#define "
                        "%s_PACKET_KIND(e, s) s\n%s_PACKET_KINDS\n#undef %s_PACKET_KIND\n};\n\n",
                        group_buffer_lower, group_buffer, group_buffer_upper, group_buffer_upper,
                        group_buffer_upper);
            ...
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail,
                        "default:\n{\nprintf(MESSAGE_CONCAT_WARN(\"Unpacking %%s not "
                        "implemented\\n\"), %s_packet_names[packet_kind]);\n}\n}\n}\n",
                        group_buffer_lower);
        } break;
        case PO_Packet_Enter: {
            ...
            if (state_stack[state_stack_tail].subcode.kind) {
                ...
                opcode_sum = (opcode_sum << shift_count)
                             | number_token_to_u32(state_stack[state_stack_tail].subcode);
            }

            output_buffers->define_buffer_tail += sprintf(
                output_buffers->define_buffer + output_buffers->define_buffer_tail,
                "#define %s_%s_ID %#llx\n", group_buffer_upper, packet_buffer_upper, opcode_sum);

            output_buffers->kind_buffer_tail +=
                sprintf(output_buffers->kind_buffer + output_buffers->kind_buffer_tail,
                        "%s_PACKET_KIND(%s_Packet_Kind_%s, \"%s\"), \\\n", group_buffer_upper,
                        group_buffer, packet_buffer, packet_buffer);

            output_buffers->registered_ids_buffer_tail += sprintf(
                output_buffers->registered_ids_buffer + output_buffers->registered_ids_buffer_tail,
                "[%s_Packet_Kind_%s] = %#llx,\n", group_buffer, packet_buffer, opcode_sum);

            output_buffers->packer_buffer_tail +=
                sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail,
                        "case %s_Packet_Kind_%s:\n{\n", group_buffer, packet_buffer);
            ...
            if (will_generate_struct) {
                output_buffers->main_struct_buffer_tail += sprintf(
                    output_buffers->main_struct_buffer + output_buffers->main_struct_buffer_tail,
                    "\ntypedef struct %s_Packet_%s %s_Packet_%s;\nstruct %s_Packet_%s\n{\n",
                    group_buffer, packet_buffer, group_buffer, packet_buffer, group_buffer,
                    packet_buffer);
        ...
        case PO_Packet_Exit: {
            output_buffers->packer_buffer_tail += sprintf(
                output_buffers->packer_buffer + output_buffers->packer_buffer_tail, "} break;\n\n");
        ...
        case PO_Field_Vec4: {
            ...
            output_buffers->packer_buffer_tail += sprintf(
                ...
                token_names[state_stack[state_stack_tail].data_type.kind], identifier_buffer);
            ...
            output_buffers->unpacker_buffer_tail += sprintf(
                output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail, "// %s %s\n",
                token_names[state_stack[state_stack_tail].data_type.kind], identifier_buffer);
            ...
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail,
                        ...
                        identifier_buffer, token_names[state_stack[state_stack_tail].data_type.kind],
                        token_names[state_stack[state_stack_tail].endian]);
        ...
        case PO_Field_Basic: {
            ...
            output_buffers->packer_buffer_tail +=
                sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail,
                        "printf(\"-- %-24s\\t%%lld\\t%%llxh\\t%%f\\n\", (i64)", identifier_buffer);

            output_buffers->packer_buffer_tail +=
                write_field_parents(output_buffers->packer_buffer, output_buffers->packer_buffer_tail,
                                    state_stack, state_stack_tail, depth);
            output_buffers->packer_buffer_tail +=
                sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail, "%s, (u64)",
                        identifier_buffer);
            ...
            output_buffers->packer_buffer_tail +=
                write_field_parents(output_buffers->packer_buffer, output_buffers->packer_buffer_tail,
                                    state_stack, state_stack_tail, depth);
            output_buffers->packer_buffer_tail +=
                sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail, "%s, (f64)",
                        identifier_buffer);
            ...
            output_buffers->packer_buffer_tail +=
                write_field_parents(output_buffers->packer_buffer, output_buffers->packer_buffer_tail,
                                    state_stack, state_stack_tail, depth);
            output_buffers->packer_buffer_tail +=
                sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail, "%s);\n\n",
                        identifier_buffer);
            ...
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail,
                        "%s, (f64)", identifier_buffer);

            output_buffers->unpacker_buffer_tail += write_field_parents(
                output_buffers->unpacker_buffer, output_buffers->unpacker_buffer_tail, state_stack,
                state_stack_tail, depth);
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail, "%s",
                        identifier_buffer);
        ...
        case PO_Field_String: {
            ...
            output_buffers->packer_buffer_tail +=
                sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail,
                        "for (u32 %s_iter = 0; %s_iter < ", identifier_buffer, identifier_buffer);
            ...
            // output_buffers->unpacker_buffer_tail, state_stack, state_stack_tail, depth - 1);
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail,
                        "printf(\"-- %-24s\\t%%d\\n\", ", "STRING_LENGTH");
            ...
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail,
                        ...
                        token_names[state_stack[state_stack_tail].length_type], identifier_buffer,
                        identifier_buffer);
        ...
        case PO_List_Enter: {
            ...
            if (!state_stack[state_stack_tail].count_specifier.kind) {
                ...
                output_buffers->packer_buffer_tail += write_field_parents(
                    output_buffers->packer_buffer, output_buffers->packer_buffer_tail, state_stack,
                    state_stack_tail, depth - 1);
                output_buffers->packer_buffer_tail +=
                    sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail,
                            "%s_count);\noffset += sizeof(%s);\n", identifier_buffer,
                            token_names[state_stack[state_stack_tail].length_type]);
            ...
            output_buffers->packer_buffer_tail +=
                write_field_parents(output_buffers->packer_buffer, output_buffers->packer_buffer_tail,
                                    state_stack, state_stack_tail, depth - 1);
            output_buffers->packer_buffer_tail +=
                sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail,
                        "%s_count; %s_iter++)\n{\n", identifier_buffer, identifier_buffer);
            ...
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail,
                        "%s_count * sizeof(", identifier_buffer);
        ...
        case PO_Stream_Enter: {
            ...
            // output_buffers->packer_buffer_tail += sprintf(output_buffers->packer_buffer +
            // output_buffers->packer_buffer_tail,
            //"endian_write_%s_%s(buffer + offset, ",
            // token_names[state_stack[state_stack_tail].length_type],
            // token_names[state_stack[state_stack_tail].endian]);
            //
            // output_buffers->packer_buffer_tail += write_field_parents(output_buffers->packer_buffer,
            // output_buffers->packer_buffer_tail, state_stack, state_stack_tail, depth - 1);
            // output_buffers->packer_buffer_tail += sprintf(output_buffers->packer_buffer +
            // output_buffers->packer_buffer_tail,
            //"%s_length);\noffset += sizeof(%s);\n",
            // identifier_buffer,
            ...
            output_buffers->packer_buffer_tail +=
                sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail,
                        "u32 will_pack_%s = ", identifier_buffer);
            ...
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail,
                        "%s_length * sizeof(", identifier_buffer);
            ...
            output_buffers->unpacker_buffer_tail +=
                sprintf(output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail,
                        "%s_length > (%s)0 ? (%s)1 : (%s)0)", identifier_buffer,
                        token_names[state_stack[state_stack_tail].length_type],
                        token_names[state_stack[state_stack_tail].length_type],
                        token_names[state_stack[state_stack_tail].length_type]);
        ...
        case PO_Switch_Enter: {
            ...
            output_buffers->unpacker_buffer_tail += sprintf(
                output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail, "switch(");
            ...
            output_buffers->packer_buffer_tail +=
                sprintf(output_buffers->packer_buffer + output_buffers->packer_buffer_tail, "switch(");
        ...
        case PO_Case_Exit: {
            ...
            output_buffers->unpacker_buffer_tail += sprintf(
                output_buffers->unpacker_buffer + output_buffers->unpacker_buffer_tail, "} break;\n");
...
int main(int argc, char* argv[]) {
    ...
    Keyword_Hash_Map keyword_map = {
        .capacity = MAX_KEYWORD_HASH_MAP_BUCKETS,
    };
    ...
    memcpy(output_buffer.data + output_buffer.tail, output_buffers->define_buffer,
           output_buffers->define_buffer_tail);
    output_buffer.tail += output_buffers->define_buffer_tail;
    memcpy(output_buffer.data + output_buffer.tail, output_buffers->kind_buffer,
           output_buffers->kind_buffer_tail);
    output_buffer.tail += output_buffers->kind_buffer_tail;
    memcpy(output_buffer.data + output_buffer.tail, output_buffers->registered_ids_buffer,
           output_buffers->registered_ids_buffer_tail);
    output_buffer.tail += output_buffers->registered_ids_buffer_tail;
    memcpy(output_buffer.data + output_buffer.tail, output_buffers->helper_struct_buffer,
           output_buffers->helper_buffer_tail);
    output_buffer.tail += output_buffers->helper_buffer_tail;
    memcpy(output_buffer.data + output_buffer.tail, output_buffers->main_struct_buffer,
           output_buffers->main_struct_buffer_tail);
    output_buffer.tail += output_buffers->main_struct_buffer_tail;
    memcpy(output_buffer.data + output_buffer.tail, output_buffers->packer_buffer,
           output_buffers->packer_buffer_tail);
    output_buffer.tail += output_buffers->packer_buffer_tail;
    memcpy(output_buffer.data + output_buffer.tail, output_buffers->unpacker_buffer,
           output_buffers->unpacker_buffer_tail);
