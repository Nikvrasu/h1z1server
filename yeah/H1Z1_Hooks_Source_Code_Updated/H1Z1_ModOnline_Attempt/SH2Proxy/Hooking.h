...
namespace hook
{
	...
	{
#ifdef _WINDOWS
		set_base((uintptr_t)GetModuleHandle(0));
#else
		set_base((uintptr_t)dlopen(NULL));
#endif
	}
...
#else
			int prot = PAGES_PROT_COMMIT;
			void *result = mmap(addr, size, prot, MAP_PRIVATE | MAP_ANON | MAP_FIXED, -1, 0);
...
#pragma endregion
...
#else
		mprotect(address, sizeof(value), PROT_READ | PROT_WRITE | PROT_EXEC);
#endif
		memcpy((void*)address, &value, sizeof(value));
#ifdef _WINDOWS
	...
	template<typename AddressType>
	inline void return_function_vp(AddressType address, uint16_t stackSize = 0)
	{
		if (stackSize == 0)
		{
			putVP<uint8_t>(address, 0xC3);
		}
		else
		{
			putVP<uint8_t>(address, 0xC2);
			putVP<uint16_t>((uintptr_t)address + 1, stackSize);
		}
	}
...
#define DEFINE_INJECT_HOOK(hookName, hookAddress) class _zz_inject_hook_##hookName : public hook::inject_hook { public: _zz_inject_hook_##hookName(uint32_t address) : hook::inject_hook(address) {}; ReturnType run(); }; \
	...
	template<typename TTarget, typename T>
	inline void set_call_basic(TTarget* target, T address)
	{
		*(T*)target = address;
	}
