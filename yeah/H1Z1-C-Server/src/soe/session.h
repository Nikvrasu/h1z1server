...
typedef struct zoneClient
{
    ...
    // do I need to add chunkRenderDistance from the typescript server? probably not, but will keep this
comment just in case I change my mind; u32 routineCounter; u32 zonePings; b8 properlyLoggedOut; // FALSE
    ...
    u32 flaggedShots;           // 0
    b8 isFairPlayFlagged;       // FALSE, keep in my that I will most likely do a proper version of
server side anticheat than convert h1emu's fairplay method? DamageInfo *damageInfo;
    // string array
    char **managedObjects;
...
struct SessionState {
    ...
    // Channel 1
    FragmentPool inputPool1;
    FragmentPool outputPool1;
    SOEInputStream inputStream1;
    SOEOutputStream outputStream1;

    // Channel 2
    FragmentPool inputPool2;
    ...
    FragmentPool outputPool4;
    SOEInputStream inputStream4;
    SOEOutputStream outputStream4;

    // Channel 5
    FragmentPool inputPool5;
