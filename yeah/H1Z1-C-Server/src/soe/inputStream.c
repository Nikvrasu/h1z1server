...
void InputStreamChannelDataParse(AppState* app, SessionState* session, SOEInputStream* input, u8* data,
                                 u32 dataLen) {
    u32 offset = 0;
    ...
    if (entry.dataLen) {
        if (!entry.isFragment) {
            ...
            if (input->pool->bufferTail == input->pool->bufferTargetLen) {
                ...
                InputStreamChannelDataParse(app, session, input, entry.data,
                                            input->pool->bufferTargetLen);
                FragmentAdvance(input->pool);
            }
        }
    }
...
SOEInputStream InputStreamInit(FragmentPool* pool, u8* rc4KeyDecoded, u32 rc4KeyLen,
                               b32 useEncryption) {
