void OutputStreamWrite(AppState* app, SessionState* session, SOEOutputStream* output, u8* data,
                       u32 dataLen, b32 overrideEncryption) {
    if (output->useEncryption && !overrideEncryption) {
        crypt_rc4_transform(&output->rc4, data, dataLen);

        // SOE protocol: if the first encrypted byte is 0x00, prepend an extra
    ...
    if (dataLen <= output->pool->packetLen) {
        ...
        ASSERT((MAX_PACKET_LENGTH - DATA_HEADER_LENGTH) == (512 - 4)
               && (MAX_PACKET_LENGTH - DATA_HEADER_LENGTH) == output->pool->packetLen);
...
SOEOutputStream OutputStreamInit(FragmentPool* pool, u8* rc4KeyDecoded, u32 rc4KeyLen,
                                 b32 useEncryption) {
